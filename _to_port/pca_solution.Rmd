...
Download the image `ds114_sub009_t2r1.nii` if you don’t already have
...
($\vec{r}$ defines same line as $-\vec{r}$) you may need to scale
...
We now need to calculate the scalar projections $c$ for each component
$\hat{u}$ and each voxel (each $\vec{v}$).

This will give us a new output matrix of scalar projections $C$ of shape
